  ...
  private _renderDiscussions(items: IDiscussion[]): void {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (items[i].ParentItemID == null) {
        ...
        //build a snippet containing any replies
        let htmlSub: string = '';
        ...
        html += `<div class="${styles.row}">
          <div class="${ styles.column}"><div class="${styles.title}">` + items[i].Title + `</div><div class="${styles.description}">` + items[i].Body.toString() + `</div></div>
          <div class="${styles.column}"><ul>` + htmlSub + `</ul><a class="${styles.button}" href="https://anitaborgo365.sharepoint.com/sites/PartnerPortal/Lists/Discussions/Flat.aspx?RootFolder=%2Fsites%2FPartnerPortal%2FLists%2FDiscussions%2FMy%20Thoughts%20on%20Topic%20A">Join the discussion</a></div>
          </div>`;
